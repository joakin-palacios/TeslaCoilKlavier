...
typedef enum {
    PICOTTS_LANG_EN_US = 0,
    PICOTTS_LANG_DE_DE,
    PICOTTS_LANG_ES_ES,
} picotts_language_t;

/**
 * Select the language resources used by the next picotts_init() call.
 * This is only meaningful when CONFIG_PICOTTS_MULTILANG is enabled.
 * The engine must be shut down before changing language.
 */
